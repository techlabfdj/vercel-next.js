...
**Defaults to `true`.** The default scrolling behavior of `<Link>` **is to follow the default browser heuristic of backwards and forwards navigation, which is to maintain the scroll position. Next.js will follow this default, as long as the [Page](/docs/app/building-your-application/routing/pages) is visible in the viewport.** If the Page is not visible in the viewport, Next.js will scroll to the top of the first Page element. When `scroll = {false}`, Next.js will continue to follow this browser heuristic, but will _not_ attempt additional scrolling behavior on navigation.
